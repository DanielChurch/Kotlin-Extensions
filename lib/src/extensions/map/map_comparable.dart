...
extension ToSortedMap<K extends Comparable<K>, V> on Map<K, V> { // TODO can this be Comparable<K>
  LinkedHashMap<K, V> toSortedMap([Comparator<K> compare]) {
    return keys.toList().sortedWith(compare).let((sortedKeys) {
      return LinkedHashMap.fromIterables(sortedKeys, sortedKeys.map(get));
    });
  }
}
